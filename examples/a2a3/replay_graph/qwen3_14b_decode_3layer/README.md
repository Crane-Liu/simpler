# `qwen3_14b_decode_3layer/` — Qwen3-14B 3-layer decode (load-balanced fused attention)

Self-contained SceneTestCase port of pypto-lib
`models/qwen3/14b/decode_layer.py` entry `decode_fwd_layers` with
`_CHUNK_NLAYERS == 3`: a **fused chunk of three consecutive Qwen3-14B decode
...
| layers | 3 | fused chunk (`decode_fwd_layers` N=3) |

Per the lib's const-layer-0 stacked-fwd reference, the three layers reuse layer-0
weights (weights + paged KV pool are stacked ×3 along dim 0, one slice per
...
codegen** for `decode_fwd_layers` (`_CHUNK_NLAYERS=3`, `PTO2_MANUAL_MAX_SEQ=5500`):
...
FP32 residual, bf16 cast points) composed over the three layers with FP32 carry +
per-layer KV pools.
...
To regenerate: in pypto-lib, set `_CHUNK_NLAYERS=3`, `PTO2_MANUAL_MAX_SEQ=5500`,
build the ×3-stacked inputs, and `decode_fwd_layers.compile_for_test(...)`; then
...
The case is intended to validate replay_graph explicit per-layer boundaries:
each layer ends with `rt_graph_boundary()` so ping-pong buffer reuse is visible
in the swimlane trace.
