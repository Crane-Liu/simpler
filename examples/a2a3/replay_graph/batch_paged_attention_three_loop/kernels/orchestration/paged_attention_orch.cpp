...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
    ...
    constexpr uint64_t REPLAY_LAYERS = 3;
    constexpr uint32_t MAX_LAYER_DONE_DEPS = 1024;
    uint64_t num_chunks = (batch + IN_CORE_BATCH - 1) / IN_CORE_BATCH;

    PTO2TaskId prev_layer_done = PTO2TaskId::invalid();
    for (uint64_t replay_layer = 0; replay_layer < REPLAY_LAYERS; replay_layer++) {
                    ...
                    for (uint64_t bn = 0; bn < max_bn; bn++) {
                        PTO2_SCOPE() {
                            ...
                            if (prev_layer_done.is_valid()) {
                                PTO2TaskId qk_deps[] = {prev_layer_done};
                                params_qk.set_dependencies(qk_deps, 1);
                            }
                            ...
                            if (layer_done_dep_count < MAX_LAYER_DONE_DEPS) {
                                layer_done_deps[layer_done_dep_count++] = up_outs.task_id();
                            } else {
                                LOG_ERROR("batch_paged_attention_two_loop: layer_done_deps overflow");
                            }
                        }
                    }
                }
        ...
        TaskOutputTensors layer_done_outs = rt_submit_dummy_task(layer_done_args);
        prev_layer_done = layer_done_outs.task_id();
        rt_graph_boundary();
    }

    LOG_INFO_V0(
        "batch_paged_attention_two_loop: replay_layers=%" PRIu64 ", task_groups=%" PRIu64
        " (batch=%" PRIu64 ", max_bn=%" PRIu64 ", chunks=%" PRIu64 ", IN_CORE_BATCH=%" PRIu64 ")",
        REPLAY_LAYERS, static_cast<uint64_t>(REPLAY_LAYERS * q_loop * num_chunks * max_bn),
        batch, max_bn, num_chunks, IN_CORE_BATCH
    );
}
