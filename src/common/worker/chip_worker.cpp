...
void ChipWorker::init(
    ...
    try {
        ...
        supports_concurrent_native_prepare_fn_ =
            load_optional_symbol<SupportsConcurrentNativePrepareFn>(handle, "supports_concurrent_native_prepare_ctx");
        ...
        set_native_run_identity_fn_ =
            load_optional_symbol<SetNativeRunIdentityFn>(handle, "set_native_run_identity_ctx");
...
void ChipWorker::run_on_slot(
    ...
    if (set_native_run_identity_fn_ != nullptr && set_native_run_identity_fn_(device_ctx_, 0, 0, 0, 0) != 0) {
...
bool ChipWorker::supports_concurrent_native_prepare() const {
    return initialized_ && pipeline_contract_.pipeline_depth > 1 && supports_concurrent_native_prepare_fn_ != nullptr &&
...
ChipWorkerNativeRun ChipWorker::prepare_native_run_on_slot(
    ...
    try {
        (void)select_slot_resources(slot_id);
        if (set_native_run_identity_fn_ != nullptr) {
