...
| Task storage | Fixed `Task[]` array | Ring buffer (`PTO2TaskDescriptor[]`) | Single-shot task window |
...
| Concurrent build+schedule | No | Yes (always) | No; scheduler starts after graph freeze |
| Profiling support | Basic | Multi-level hierarchy | Multi-level hierarchy |
| Batch/streaming | No | Yes (flow control, back-pressure) | Whole graph must fit configured capacities |
| Thread model | N scheduler threads | 1 orchestrator + 3 schedulers | 1 orchestrator, then 3 schedulers |
| Use case | Development, debugging | Production workloads | Single-shot replay workloads |
...
The replay runtime builds the complete graph on AICPU before scheduler dispatch begins. It uses one logical ring, monotonic task/output/dependency allocation, and an immutable fanout graph built directly by `submit_task`.

- Task lifecycle: `PENDING -> COMPLETED`; no `CONSUMED` or in-replay reclaim
- Capacity contract: the entire graph must fit the task window, heap, dependency pool, and TensorMap pool
- Synchronization: orchestrator publishes the frozen graph through a release/acquire barrier before schedulers seed initial-ready tasks
- Failure mode: capacity exhaustion is a fatal sizing error rather than back-pressure

See [replay_graph/docs/RUNTIME_LOGIC.md](../runtime/replay_graph/docs/RUNTIME_LOGIC.md) for the full design.
