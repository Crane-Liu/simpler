...
static bool append_fanin_or_fail(
    ...
    if (prod_state->task_state.load(std::memory_order_acquire) >= PTO2_TASK_COMPLETED ||
        pto2_is_fanout_closed(prod_state->fanout_head.load(std::memory_order_acquire))) {
        // Already-completed producer: the dependency is already satisfied, so it
        // contributes nothing to the consumer's readiness. fanin_count /
        // fanin_refcount / dispatch_fanin all track only still-pending producers.
        return true;
    }
    ...
    // completion releases it. Completion atomically closes the fanout list; a
    // concurrent append either wins before close and is released by the scheduler,
    // or observes CLOSED and treats the producer as already satisfied.
    PTO2DepListEntry *new_entry = orch->dep_pool.alloc();
    if (!new_entry) {
        orch_mark_fatal(orch, PTO2_ERROR_DEP_POOL_OVERFLOW);
        return false;
    }
    new_entry->slot_state = consumer;

    PTO2DepListEntry *head = prod_state->fanout_head.load(std::memory_order_acquire);
    while (true) {
        if (pto2_is_fanout_closed(head) ||
            prod_state->task_state.load(std::memory_order_acquire) >= PTO2_TASK_COMPLETED) {
            return true;
        }
        new_entry->next = head;
        if (prod_state->fanout_head.compare_exchange_weak(
                head, new_entry, std::memory_order_acq_rel, std::memory_order_acquire
            )) {
...
static bool prepare_task(
    ...
    // The publish gate keeps newly submitted tasks unrunnable until the current
    // graph boundary. Real producer dependencies are added on top of this gate
    // by submit_task_common's fanin loop.
    ...
    out->slot_state->any_subtask_deferred.store(false, std::memory_order_relaxed);
    ...
    // fanin_count / fanin_refcount start with the graph publish gate above.
...
static TaskOutputTensors submit_task_common(
    ...
    // Readiness is gated by graph publish. rt_graph_boundary releases that
    // synthetic fanin after all tasks in the graph have been fully materialized.
