...
void SchedulerContext::log_stall_diagnostics(
    ...
    if (thread_idx == 0) {
        ...
        PTO2SharedMemoryHeader &graph = *sched_->sm_header;
        int32_t submitted = graph.fc.task_count.load(std::memory_order_relaxed);
        for (int32_t si = 0; si < submitted; si++) {
            PTO2TaskSlotState &slot_state = graph.get_slot_state_by_task_id(si);
        ...
        int32_t effective_total = task_count > 0 ? task_count : submitted;
...
SchedulerContext::StallClassification SchedulerContext::classify_stall_reason() const {
    ...
    PTO2SharedMemoryHeader &graph = *sched_->sm_header;
    int32_t task_count = graph.fc.task_count.load(std::memory_order_relaxed);
...
void SchedulerContext::log_l2_swimlane_summary(int32_t thread_idx, [[maybe_unused]] int32_t cur_thread_completed) {
...

    {
        ...
        uint64_t otc_total = sp.fanout_cycle;
...
int32_t SchedulerContext::post_handshake_init(Runtime *runtime) {
    ...
    if (runtime->get_gm_sm_ptr()) {
        ...
        int32_t count = header->fc.task_count.load(std::memory_order_acquire);
        total_tasks_ = (count > 0 && count <= PTO2_SCOPE_TASKS_CAP) ? count : 0;
    } else {
        total_tasks_ = 0;
    }
