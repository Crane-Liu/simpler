...
int32_t SchedulerContext::resolve_and_dispatch(Runtime *runtime, int32_t thread_idx) {
    ...
    // replay_graph pipeline: scheduler threads start polling immediately after
    // one-time init. The orchestrator publishes initial-ready tasks at each
    // graph boundary (and once more at final orchestration_done), so there is no
    // whole-graph barrier or one-shot seed here.
    (void)pto2_seed_claimed_;
...
#endif
                record_graph_task_complete(dummy_slot);
#if PTO2_PROFILING
