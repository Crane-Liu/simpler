...
void SchedulerContext::record_graph_task_complete(PTO2TaskSlotState &slot_state) {
    if (rt_ == nullptr || slot_state.task == nullptr) {
        return;
    }
    int32_t task_id = static_cast<int32_t>(slot_state.task->task_id.local());
    PTO2ReplayGraphPipelineState &pipeline = rt_->graph_pipeline;
    for (int32_t i = 0; i < PTO2_REPLAY_GRAPH_BUFFER_COUNT; i++) {
        PTO2ReplayGraphBufferControl &buf = pipeline.buffers[i];
        int32_t begin = buf.task_begin;
        int32_t end = begin + buf.task_count;
        if (task_id < begin || task_id >= end) {
            continue;
        }
        int32_t done = buf.completed_count.fetch_add(1, std::memory_order_acq_rel) + 1;
        if (done >= buf.task_count) {
            buf.exec_done.store(1, std::memory_order_release);
            if (!pto2_try_release_graph_buffer(buf)) {
