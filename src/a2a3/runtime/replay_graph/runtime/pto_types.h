...
#ifndef SRC_A2A3_RUNTIME_REPLAY_GRAPH_RUNTIME_PTO_TYPES_H_
#define SRC_A2A3_RUNTIME_REPLAY_GRAPH_RUNTIME_PTO_TYPES_H_
 ...
 *
 * LIFETIME — current replay:
 *   Internally this class stores pointers into the submitting task's payload
 *   (PTO2TaskPayload::tensors[]), which lives in a single-use graph slot.
 *   Scopes do not invalidate it. The storage remains stable until the pooled
 *   arena is reset for the next replay; references must not cross invocations.
 */
...
#endif  // SRC_A2A3_RUNTIME_REPLAY_GRAPH_RUNTIME_PTO_TYPES_H_
