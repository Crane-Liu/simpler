...
struct PTO2RuntimeOps {
    ...
    // Graph/layer boundary hook. Publishes the just-built graph to S, releases
    // its synthetic publish gates, and switches O to a safe ping-pong buffer.
    void (*graph_boundary)(PTO2Runtime *rt);
};

enum PTO2ReplayGraphBufferState : int32_t {
    PTO2_GRAPH_BUFFER_FREE = 0,
    PTO2_GRAPH_BUFFER_BUILDING = 1,
    PTO2_GRAPH_BUFFER_SEALED = 2,
    PTO2_GRAPH_BUFFER_RUNNING = 3,
    PTO2_GRAPH_BUFFER_DONE = 4,
};
...
struct PTO2ReplayGraphPipelineState {
    std::atomic<int32_t> enabled;
    std::atomic<int32_t> all_done;
    std::atomic<int32_t> published_task_count;
    int32_t active_buffer;
    int32_t graph_count;
    int32_t published_initial_ready_count;
    uint64_t current_graph_epoch;
    PTO2ReplayGraphBufferControl buffers[PTO2_REPLAY_GRAPH_BUFFER_COUNT];
};

inline bool pto2_try_release_graph_buffer(PTO2ReplayGraphBufferControl &buf) {
    if (buf.exec_done.load(std::memory_order_acquire) == 0 ||
        buf.dep_closed.load(std::memory_order_acquire) == 0) {
        return false;
    }
    buf.state.store(PTO2_GRAPH_BUFFER_FREE, std::memory_order_release);
    return true;
}
...
struct PTO2Runtime {
    ...
    // Graph-level pipeline state used to pipeline O/S across graph boundaries
    // and gate ping-pong buffer reuse.
...
/**
 * Publish the current graph/layer for replay_graph pipeline mode. The boundary
 * releases the graph's synthetic fanin gates, lets S start executing, then waits
 * for the next ping-pong buffer to become reusable before O builds into it.
 */
