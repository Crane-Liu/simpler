 ...
 * layout (the input to runtime_reserve_layout). Stable per callable/config;
 * re-read at AICPU boot to reconstruct whole-graph capacities.
 */
...
PTO2RuntimeArenaLayout runtime_reserve_layout(
    DeviceArena &arena, uint64_t task_window_size, uint64_t heap_size,
 ...
 * End current scope
 *
 * Ends the current dependency/profiling scope. No task or buffer is reclaimed.
 */
