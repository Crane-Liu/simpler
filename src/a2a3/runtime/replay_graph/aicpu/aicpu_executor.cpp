    ...
    if (thread_idx >= sched_thread_num_) {
        ...
        // scope the per-callable dlopen / SO-table locals to this block.
        {
            ...
            // re-wires arena-internal pointers to device addresses below.
            {
                ...
                LOG_INFO_V0(
                    "Thread %d: Replay graph sizes: task_window=%" PRIu64 " heap=%" PRIu64 " dep_pool=%d", thread_idx,
                    rt->prebuilt_layout.sizing.task_window_size, rt->prebuilt_layout.sizing.heap_size,
                    rt->prebuilt_layout.sizing.dep_pool_capacity
                );
            }
            ...
            // controlled by graph completion and dependency-closure state.
            {
                ...
                if (!rt->sm_handle->init(
                        ...
                        rt->prebuilt_layout.sizing.heap_size
                    )) {
                ...
                rt->orchestrator.l2_swimlane_level = get_l2_swimlane_level();
                {
                    ...
                    scope_stats_set_ring_capacity(
                        0, alloc.window_size(), alloc.heap_capacity(), rt->prebuilt_layout.sizing.dep_pool_capacity
                    );
            ...
            LOG_INFO_V9(
                "Thread %d:   task+heap_alloc: %.3fus (%.1f%%)", thread_idx, cycles_to_us(p.alloc_cycle),
            ...
            LOG_INFO_V9(
                "Thread %d:   fanin+ready    : %.3fus (%.1f%%)", thread_idx, cycles_to_us(p.fanin_cycle),
